...
//get letter when keyboard is pressed---------------------------------
addEventListener("keydown", (event) => {
  ...
  if ((isletter.test(event.key)) && (event.key.length == 1)) {
    ...
    if (boxTraker >= 4){
     ...
     //print in box
    //document.getElementById("test2").innerHTML = newBox; //test
    //document.getElementById("test3").innerHTML = inputWord; //test
...
async function init(){
  ...
  document.getElementById('test').innerHTML = wordRes; //test
  findWord = wordRes;
}
...
async function checkWord(){
  ...
  for (let i=0; i < WORD_LENGTH; i++){
    var tempbox = 'box'+ (rowTraker-1) + i;
    //document.getElementById('test4').innerHTML = tempbox; //test
