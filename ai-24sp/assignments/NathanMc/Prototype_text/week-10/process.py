...
from DummyGTP import DummyGPTModel
...
# Pg 113: Implement DummyGTP
batch = []
txt1 = "In the midst of chaos, there is also opportunity."
txt2 = "The greatest victory is that which requires no battle."

encoded_txt1 = torch.tensor(tokenizer.encode(txt1))
encoded_txt2 = torch.tensor(tokenizer.encode(txt2))

# Pad the sequances to the length of the longest one
max_len = max(encoded_txt1.size(0), encoded_txt2.size(0))
padded_txt1 = torch.nn.functional.pad(encoded_txt1, (0, max_len - encoded_txt1.size(0)), 'constant', 0)
padded_txt2 = torch.nn.functional.pad(encoded_txt2, (0, max_len - encoded_txt2.size(0)), 'constant', 0)

batch.append(padded_txt1)
batch.append(padded_txt2)
batch = torch.stack(batch, dim=0)

print("Dummy GTP Batch:\n", batch)

# Pg 114: Initialize a new 124 million parameter DUmmyGPTModel instance
...
# Pg 116: create 2 training examples with 5 dimensions each
...
# Mean and Variance
# Pg 118: Next, apply layer normalization to the layer outputs
#mean = out.mean(dim=-1, keepdim=True)
#variance = out.var(dim=-1, keepdim=True)
#out_norm = (out - mean) / torch.sqrt(variance + 1e-5)  # Applying epsilon for numerical stability
torch.set_printoptions(sci_mode=False)

# Pg 120: Layer Normalization
